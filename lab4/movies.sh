...
function print_help () {
    ...
    echo -e "-y YEAR\n\tSearch movies that were released in YEAR"
    echo -e "-R QUERY\n\tSearch movies with QUERY in plot"
    echo -e "-i \n\tSearch ignores case while using -R"
...
function query_plot () {
    # Returns list of movies from ${1} with query ${2} matching plot slot
    local -r MOVIES_LIST=${1}
    local -r QUERY=${2}
    ...
    for MOVIE_FILE in ${MOVIES_LIST}; do
        if ${IGNORE_CASE:-false}; then
            if grep "| Plot" "${MOVIE_FILE}" | grep -q -i "${QUERY}"; then
                RESULTS_LIST+=( "${MOVIE_FILE}" )
            fi
        else
...
IGNORE_CASE=false
while getopts ":hd:t:a:f:y:R:ix" OPT; do
  case ${OPT} in
    h)
        print_help
        exit 0
...
if ${SEARCHING_IN_PLOT:-false}; then
